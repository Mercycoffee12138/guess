...
void hash_worker_thread(const unordered_set<string>& test_set, double& time_hash) {
    ...
    while (!hash_thread_should_exit) {
        ...
        if (!local_guesses.empty()) {
            // 计算完整的4密码批次数量和剩余密码数量
            size_t complete_batches = local_guesses.size() / 4;
            size_t remainder = local_guesses.size() % 4;
            
            // 处理完整的4密码批次
            #pragma omp parallel for reduction(+:total_cracked)
            for (size_t i = 0; i < complete_batches; i++) {
                string inputs[4];
                bit32 states[4][4];
                
                // 准备4个密码
                for (int j = 0; j < 4; j++) {
                    inputs[j] = local_guesses[i*4 + j];
                }
                ...
                // 检查是否破解
                for (int j = 0; j < 4; j++) {
                    if (test_set.find(inputs[j]) != test_set.end()) {
                        #pragma omp atomic
                        total_cracked++;
                    }
                }
            ...
            if (remainder > 0) {
                ...
                for (size_t i = 0; i < remainder; i++) {
                    inputs[i] = local_guesses[complete_batches*4 + i];
                }
