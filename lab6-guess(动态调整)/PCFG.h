...
public:
    ...
    void GenerateGPUAsync(PT pt, cudaStream_t stream, vector<char>& h_results);
    void GenerateGPUAsync_SingleSegment(PT pt, cudaStream_t stream, vector<char>& h_results);
    void GenerateGPUAsync_MultiSegment(PT pt, cudaStream_t stream, vector<char>& h_results);

    void PopNextBatch(int batch_size);

    int EstimateWorkload(const PT& pt) {
        if (pt.content.size() == 1) {
            // 单segment情况
            return pt.max_indices[0];
        } else {
            // 多segment情况，考虑最后一个segment可能值数量
            int last_idx = pt.content.size() - 1;
            return pt.max_indices[last_idx];
        }
    }

    void ProcessPTAdaptively(PT pt);
    int AutoTuneThresholds();
    void HybridPopNextBatch(int batch_size);
    // 将优先队列最前面的一个PT
    void PopNext();
    int total_guesses = 0;
