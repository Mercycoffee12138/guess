...
public:
    ...
    std::queue<ThreadGenerateArgs*> task_queue; // Stores pointers to ThreadGenerateArgs
    pthread_mutex_t pool_mutex;
    pthread_cond_t pool_task_available_cond;
    pthread_cond_t pool_tasks_all_done_cond;
    bool pool_shutdown_flag;
    volatile int pool_pending_tasks_count; // Number of tasks currently being processed or in queue for this Generate call

    // Thread pool methods (declarations if definitions are in .cpp)
    void init_thread_pool(int num_threads);
    void shutdown_thread_pool();
    static void* worker_thread_function(void* arg); // Definition is in guessing.cpp
};
